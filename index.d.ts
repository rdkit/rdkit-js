...
declare module RDKitModule {
    ...
    interface Mol {
        ...
        // coord generation
        generate_aligned_coords(templateMol: Mol): string
        generate_aligned_coords(templateMol: Mol, useCoordGen: boolean): string
        generate_aligned_coords(templateMol: Mol, useCoordGen: boolean, allowOptionalAttachements: boolean): string
        generate_aligned_coords(templateMol: Mol, useCoordGen: boolean, allowOptionalAttachements: boolean, acceptFailure: boolean): string
        is_valid(): boolean
        has_coords(): boolean
    ...
    function get_mol(input: string, details_json: string): Mol
    function get_mol_from_pickle(pkl: string): Mol
    function get_mol_from_uint8array(pklAsUInt8Array: Uint8Array): Mol
    function get_mol_copy(other: Mol): Mol
    function get_qmol(input: string): Mol
    function get_inchikey_for_inchi(inchi: string): string

    function version(): string
    function prefer_coordgen(prefer: boolean): void
    function use_legacy_stereo_perception(value: boolean): void
}

export default RDKitModule