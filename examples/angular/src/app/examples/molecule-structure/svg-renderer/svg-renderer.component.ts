...
export class SvgRendererComponent implements OnChanges, AfterViewInit {
  ...
  @ViewChild("molSVG", { read: ElementRef })
  svgContainer!: ElementRef<HTMLDivElement>;

  safeSVG?: SafeHtml;

  constructor(
    private rdkit: RDKitLoaderService,
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      Object.keys(changes).every((k) => {
        if (typeof changes[k].currentValue == "object") {
          return (
            JSON.stringify(changes[k].currentValue) ==
            JSON.stringify(changes[k].previousValue)
          );
        }
    ...
    setTimeout(() => this.renderMolecule());
  }
      ...
      this.rdkit
        ...
        .subscribe((rdkit) => {
          ...
          try {
            ...
            const svgData = mol.get_svg_with_highlights(
