...
import {
  DrawColour,
  MolDrawOptions
} from "../../molecule-structure/mol-draw-options";
...
export class DrawingOptionsComponent {
  ...
  constructor() {
    this.drawOptionsForm = new FormGroup({
      mainStructureInput: new FormControl(
      ...
      highlightColour: new FormControl("#fd5c63", [
        Validators.pattern("#[a-fA-F0-9]{6}")
      ]),
      legendColour: new FormControl("#000000", [
        Validators.pattern("#[a-fA-F0-9]{6}")
      ]),
      symbolColour: new FormControl("#000000", [
        Validators.pattern("#[a-fA-F0-9]{6}")
      ]),
      backgroundColour: new FormControl("#ffffff", [
        Validators.pattern("#[a-fA-F0-9]{6}")
      ]),
    ...
    this.drawOptionsForm.valueChanges
  ...
  parseFormValue(formValue: any) {
    ...
    const l = temp.highlightColour.match(
  ...
  parseRGBToDrawColour(rgb: string): DrawColour {
    return rgb
      ...
      ?.slice(1) as DrawColour; // Regex includes the entire match as the first in the list, so ignore the first
  }
}
