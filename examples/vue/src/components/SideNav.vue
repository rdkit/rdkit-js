<template>
    <template v-for="section of sections">
        ...
        <ul class="menu-list">
            <li v-for="child of section.children">
                <a :href="child.id" :class="activeLink === child.id ? 'is-active' : undefined" @click="activeLink = child.id">{{ child.name }}</a>
            </li>
        </ul>
    </template>
</template>
...
let activeLink = ref('#component-introduction')
